...
%% set up data
function [ss_energy coefTensor InsFreq] = SS_WP_Yang(filename)
...
%     colorbar off;colormap (jet);caxis([0 0.7]);
...
%     colorbar off;colormap (jet);caxis([0 0.05]);
...
if 1
    ...
    % show results
    pic = figure;imagesc([0 timeEnd],[lowBound upBound],(ss_energy.^(1/3)));xlabel('Time (Second)');ylabel('Frequency (Hz)');axis xy;;
    colorbar off;colormap (jet);caxis([0 0.7]);
...
%     % show results
%     pic=figure;imagesc([0 timeEnd],[lowBound upBound/2],ss_energy);axis square;xlabel('Time (Second)');ylabel('Frequency (Hz)');axis xy;;
%     colorbar off;colormap (jet);caxis([0 0.04]);
...
%     % show results
%     pic=figure;imagesc([0 timeEnd],[lowBound upBound/2],ss_energy);axis square;xlabel('Time (Second)');ylabel('Frequency (Hz)');axis xy;;
%     colorbar off;colormap (jet);caxis([0 0.02]);
